...
const columns: Column<any>[] = [
    ...
    { key: 'reference_number', label: 'Nomor Referensi', render: (val) => (val as string) || '—' },
    { key: 'details_count', label: 'Jumlah Item', render: (val) => `${val} Item` },
    { key: 'creator', label: 'Dikeluarkan Oleh', render: (val, row) => row.creator?.name || '—' },
...
const viewDetails = async (row: any) => {
    const res = await fetch(`/api/inventory-outs/${row.id}`, { headers: { Accept: 'application/json' } });
    if (res.ok) {
...
const addRow = () => {
    formDetails.value.push({ inventory_item_id: '', qty_good: 0, qty_fair: 0, qty_damaged: 0 });
};
...
const resetForm = () => {
    ...
    formRefNum.value = '';
    formNotes.value = '';
    formDetails.value = [{ inventory_item_id: '', qty_good: 0, qty_fair: 0, qty_damaged: 0 }];
    selectedFiles.value = [];
    formErrors.value = {};
};
