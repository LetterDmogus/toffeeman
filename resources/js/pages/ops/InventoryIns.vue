...
<template>
    <div class="p-6">
        <CRUDTable
            resource-name="Barang Masuk"
            api-url="/api/inventory-ins"
            :columns="columns"
            :form-fields="[]"
            disable-edit
            disable-delete
            :default-visible-columns="['date', 'reference_number', 'details_count', 'creator', 'notes']"
            auditable-type="InventoryIn"
        >
            ...
            <template #form="{ mode, submitting, cancel, refresh }">
                <div class="rounded-xl border bg-card p-6 shadow-sm animate-in fade-in duration-300">
                    <div class="flex items-center gap-3 border-b pb-4 mb-6">
                        ...
                        <div>
                            <h2 class="text-lg font-bold text-foreground">Catat Barang Masuk</h2>
                            <p class="text-xs text-muted-foreground">Isi form untuk mencatat riwayat barang masuk baru.</p>
                        </div>
                    </div>
                    ...
                    <form @submit.prevent="submitCustomForm(cancel, refresh)" class="grid gap-6">
                        <div class="grid gap-4 md:grid-cols-2">
                            <div class="flex flex-col gap-1.5">
                                <Label for="form-date">Tanggal Masuk <span class="text-destructive">*</span></Label>
                            ...
                            <div class="flex flex-col gap-1.5">
                                <Label for="form-ref">Nomor Referensi / Invoice / SJ</Label>
                                <Input id="form-ref" type="text" placeholder="Contoh: INV/2026/001" v-model="formRefNum" />
                                <p v-if="formErrors.reference_number" class="text-xs text-destructive">{{ formErrors.reference_number[0] }}</p>
                            </div>
                        </div>

                        <div class="flex flex-col gap-1.5">
                            <Label for="form-notes">Catatan Tambahan</Label>
                            <textarea
                                id="form-notes"
                                v-model="formNotes"
                                rows="3"
                                placeholder="Keterangan mengenai kiriman barang ini..."
                        ...
                        <div class="flex flex-col gap-3">
                            <div class="flex items-center justify-between border-b pb-2">
                                <h3 class="text-sm font-semibold text-foreground">Daftar Barang Masuk</h3>
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                >
                                    ...
                                    <div class="grid gap-3 sm:grid-cols-2 md:grid-cols-5 mt-2">
                                        <div class="flex flex-col gap-1 md:col-span-2">
                                            <Label class="text-xs text-muted-foreground">Harga Beli Satuan (Rp) <span class="text-destructive">*</span></Label>
                                            <Input type="number" min="0" v-model="detail.price" required class="h-9 text-xs" placeholder="Harga Beli Satuan" />
                                        </div>
                        ...
                        <div class="flex flex-col gap-1.5 border-t pt-4">
                            ...
                            <input
                                ...
                                class="text-xs file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-brand-55 file:text-brand-700 hover:file:bg-brand-100 cursor-pointer"
                            />
        ...
        <Dialog v-model:open="detailDialogOpen">
            <DialogContent class="sm:max-w-2xl max-h-[85vh] overflow-y-auto">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Informasi lengkap dan bukti fisik pengiriman barang.
                    </DialogDescription>
                </DialogHeader>
                ...
                <div v-if="detailItem" class="grid gap-6 py-2">
                    <div class="grid gap-3 sm:grid-cols-2 text-xs border-b pb-4">
                        ...
                        <div>
                            <span class="font-bold text-muted-foreground">Nomor Referensi/Invoice:</span>
                            <p class="text-foreground mt-0.5 font-mono">{{ detailItem.reference_number || '—' }}</p>
                        </div>
                        <div>
                            <span class="font-bold text-muted-foreground">Diterima Oleh:</span>
                    ...
                    <!-- Items list -->
                    <div>
                        <h4 class="text-xs font-bold text-muted-foreground uppercase mb-2">Daftar Barang</h4>
                            ...
                            <table class="w-full text-xs text-left">
                                ...
                                <tbody>
                                    <tr v-for="d in detailItem.details" :key="d.id" class="border-b last:border-0">
                                        ...
                                        <td class="px-3 py-2 text-right">Rp {{ Number(d.price).toLocaleString('id-ID') }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
