...
const refreshPositions = async () => {
    const res = await fetch('/api/positions?all=true', { headers: { Accept: 'application/json' } });
    if (res.ok) {
        const data = await res.json();
        const field = fields.find(f => f.key === 'position_id');
        if (field) field.options = data.map((p: any) => ({ value: p.id, label: p.name }));
    }
};
...
const columns: Column<any>[] = [
    ...
    { key: 'email', label: 'Email', render: (val, row) => row.user?.email || '—' },
    { key: 'position_id', label: 'Jabatan', render: (val, row) => row.position?.name || '—' },
    { key: 'salary', label: 'Gaji', render: (val) => `Rp ${Number(val).toLocaleString('id-ID')}` },
...
const fields: FormField[] = [
    ...
    { key: 'password', label: 'Password', type: 'text', placeholder: 'Isi jika ganti password' },
    { key: 'salary', label: 'Gaji Bulanan', type: 'number', required: true },
    { key: 'position_id', label: 'Jabatan', type: 'select', required: true, options: [] },
    { key: 'status', label: 'Status Staf', type: 'select', required: true, options: [{ value: 'active', label: 'Aktif' }, { value: 'inactive', label: 'Tidak Aktif' }] },
];
